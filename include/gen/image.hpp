...
class image : public resource {
	struct ___method_bindings {
		...
		godot_method_bind *mb_fill_rect;
...
public:
	...
	void create_from_data(const int64_t width, const int64_t height, const bool use_mipmaps, const int64_t format, const pool_byte_array data);
	void crop(const int64_t width, const int64_t height);
	error decompress();
	image::AlphaMode detect_alpha() const;
	void expand_x2_hq2x();
	void fill(const color color_);
	void fill_rect(const rect2 rect, const color color_);
	...
	error load_bmp_from_buffer(const pool_byte_array buffer);
	error load_jpg_from_buffer(const pool_byte_array buffer);
	error load_png_from_buffer(const pool_byte_array buffer);
	error load_tga_from_buffer(const pool_byte_array buffer);
	error load_webp_from_buffer(const pool_byte_array buffer);
	void lock();
	...
	error save_exr(const string path_, const bool grayscale = false) const;
	error save_png(const string path_) const;
	pool_byte_array save_png_to_buffer() const;
	void set_pixel(const int64_t x, const int64_t y, const color color_);
	void set_pixelv(const vector2 dst, const color color_);
	void shrink_x2();
	void srgb_to_linear();
	void unlock();
