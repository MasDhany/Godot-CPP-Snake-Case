...
public:
	...
	pool_byte_array read_response_body_chunk();
	error request(const int64_t method, const string url, const pool_string_array headers, const string body = "");
	error request_raw(const int64_t method, const string url, const pool_string_array headers, const pool_byte_array body);
	void set_blocking_mode(const bool enabled);
	void set_connection(const ref<stream_peer> connection);
