...
class navigation2_dserver : public object {
	static navigation2_dserver *_singleton;
...
public:
	...
	static inline navigation2_dserver *get_singleton()
	{
		if (!navigation2_dserver::_singleton) {
			navigation2_dserver::_singleton = new navigation2_dserver;
		}
		return navigation2_dserver::_singleton;
	}
	...
	pool_vector2_array map_get_path(const rid map, const vector2 origin, const vector2 destination, const bool optimize, const int64_t navigation_layers = 1) const;
	...
	void region_set_navpoly(const rid region, const ref<navigation_polygon> nav_poly) const;
	void region_set_transform(const rid region, const transform2d transform_) const;
