...
class os : public object {
	...
	struct ___method_bindings {
		...
		godot_method_bind *mb_get_display_cutouts;
		...
		godot_method_bind *mb_get_main_thread_id;
		...
		godot_method_bind *mb_get_processor_name;
		...
		godot_method_bind *mb_get_screen_refresh_rate;
		...
		godot_method_bind *mb_has_clipboard;
		...
		godot_method_bind *mb_is_process_running;
		...
		godot_method_bind *mb_keyboard_get_scancode_from_physical;
		godot_method_bind *mb_keyboard_set_current_layout;
		godot_method_bind *mb_kill;
		godot_method_bind *mb_move_to_trash;
...
public:
	...
	void dump_memory_to_file(const string file_);
	void dump_resources_to_file(const string file_);
	int64_t execute(const string path_, const pool_string_array arguments, const bool blocking = true, const array output = array(), const bool read_stderr = false, const bool open_console = false);
	int64_t find_scancode_from_string(const string string_) const;
	...
	pool_string_array get_cmdline_args();
	string get_config_dir() const;
	pool_string_array get_connected_midi_inputs();
	...
	pool_string_array get_granted_permissions() const;
	...
	int64_t get_main_thread_id() const;
	...
	real_t get_screen_refresh_rate(const int64_t screen = -1) const;
	...
	void global_menu_add_item(const string menu, const string label_, const variant id, const variant meta);
	...
	bool is_process_running(const int64_t pid) const;
	...
	int64_t keyboard_get_scancode_from_physical(const int64_t scancode) const;
	void keyboard_set_current_layout(const int64_t index);
	error kill(const int64_t pid);
	error move_to_trash(const string path_) const;
	void move_window_to_foreground();
	bool native_video_is_playing();
	void native_video_pause();
	error native_video_play(const string path_, const real_t volume, const string audio_track, const string subtitle_track);
	void native_video_stop();
	void native_video_unpause();
	void open_midi_inputs();
	void print_all_resources(const string tofile = "");
	void print_all_textures_by_size();
	void print_resources_by_type(const pool_string_array types);
	...
	void set_window_mouse_passthrough(const pool_vector2_array region);
