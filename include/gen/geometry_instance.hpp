...
class geometry_instance : public visual_instance {
	struct ___method_bindings {
		...
		godot_method_bind *mb_get_material_overlay;
		...
		godot_method_bind *mb_set_material_overlay;
...
public:
	...
	// methods
	geometry_instance::ShadowCastingSetting get_cast_shadows_setting() const;
	real_t get_extra_cull_margin() const;
	bool get_flag(const int64_t flag) const;
	bool get_generate_lightmap();
	geometry_instance::LightmapScale get_lightmap_scale() const;
	...
	void set_material_overlay(const ref<material> material_);
	void set_material_override(const ref<material> material_);
