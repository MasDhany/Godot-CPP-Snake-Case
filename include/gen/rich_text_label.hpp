...
class rich_text_label : public control {
	struct ___method_bindings {
		...
		godot_method_bind *mb_get_selected_text;
...
public:
	...
	static rich_text_label *_new();

	// methods
	void _gui_input(const ref<input_event> arg0);
	void _scroll_changed(const real_t arg0);
	void add_image(const ref<texture> image_, const int64_t width = 0, const int64_t height = 0, const int64_t align = 2);
	...
	dictionary parse_expressions_for_values(const pool_string_array expressions);
	void pop();
	void push_align(const int64_t align);
	void push_bold();
	void push_bold_italics();
	void push_cell();
	void push_color(const color color_);
	void push_font(const ref<font> font_);
